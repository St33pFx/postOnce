...
test("REQ-CP-007: uploaded cover can be edited without a video",async({page,context})=>{
  ...
  await expect(page.locator(".cover-thumb .cover-text")).toHaveText("Portada sin video");`r`n  await expect(page.getByRole("status")).toHaveText("Guardado en el servidor");
});
