...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try { const user = await currentUser(await headers()); if (!user) return Response.json({ error: "Unauthorized" }, { status: 401 }); const { id } = await params; const { db } = identityServices(); return Response.json(await (await publishingService(db)).retrySecondary(user.id, id), { status: 202, headers: { "Cache-Control": "no-store" } }); }
