...
export function DraftWorkspace() {
  ...
  return <main className="draft-workspace">
    ...
    {open&&<article className="editor-column"><div className="editor-heading"><div><p className="eyebrow">EDITOR</p><h2>Editar publicación</h2></div><span role="status" aria-live="polite" className={`save-state ${status}`}>{labels[status]}</span></div>
      ...
      <section className="editor-section" data-section-reveal aria-labelledby="video-heading"><div className="section-heading"><div><p className="eyebrow">01</p><h2 id="video-heading">Video</h2></div>{activeVideo&&<span className="status-chip">Video seleccionado</span>}</div>
      ...
      <section className="editor-section" data-section-reveal aria-labelledby="cover-heading"><div className="section-heading"><div><p className="eyebrow">02</p><h2 id="cover-heading">Portada</h2></div>{base&&<span className="status-chip">Portada seleccionada</span>}</div>
        {base&&edit.cover?<div className="selected-media cover-media"><div className="cover-thumb" style={{aspectRatio:`${base.metadata?.width??1}/${base.metadata?.height??1}`}}>{urls[base.id]?<Image unoptimized src={urls[base.id]} width={base.metadata?.width??1} height={base.metadata?.height??1} alt="Portada seleccionada"/>:<div className="media-placeholder">Preparando preview…</div>}</div><div className="media-details"><strong>{base.kind==="extracted_frame"?"Frame del video":"Imagen original"}</strong><p>{bytes(base.size)}{base.metadata?.width?` · ${base.metadata.width} × ${base.metadata.height}`:""}</p><div className="inline-actions"><label className="replace-control"><span>Reemplazar portada</span><input aria-label="Reemplazar portada" disabled={busy||!!upload} type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>{const f=e.target.files?.[0];if(f)void action(()=>startUpload(f,"uploaded_image"));e.target.value="";}}/></label></div></div></div>:<div className="empty-media"><p>Sube una imagen para usarla como portada general.</p><label className="button primary upload-inline">Seleccionar imagen<input aria-label="Subir imagen" disabled={busy||!!upload} type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>{const f=e.target.files?.[0];if(f)void action(()=>startUpload(f,"uploaded_image"));e.target.value="";}}/></label></div>}
