...
export function DraftWorkspace() {
  ...
  const preview = useRef<HTMLDivElement>(null), coverThumb = useRef<HTMLDivElement>(null), [previewWidth,setPreviewWidth] = useState(320), [coverThumbWidth,setCoverThumbWidth] = useState(270);
  ...
  useEffect(() => { if (!coverThumb.current) return; const observer=new ResizeObserver(entries=>setCoverThumbWidth(entries[0].contentRect.width)); observer.observe(coverThumb.current); return()=>observer.disconnect(); },[edit.cover?.baseId]);
